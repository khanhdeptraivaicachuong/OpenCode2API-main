...
> Two ways to run: **Docker** (zero-config) or **direct launch** on Windows CMD / Linux / macOS terminal. Both are first-class supported.

### 🪟🐧🍎 Direct Launch (Windows CMD / Linux / macOS — no Docker)

You need **Node.js 18.17+** and the **OpenCode CLI** installed.

#### 1. Install the OpenCode CLI (once per machine)
...
The only two values you must change are `API_KEY` and `OPENCODE_SERVER_PASSWORD`. Everything else has sensible defaults.

#### 4. Start

Pick **whichever shell you're in**:
...
If you prefer the npm command, that's the same on every platform:
...
The proxy will **auto-spawn** `opencode serve` on port `10001` (because `MANAGE_BACKEND` defaults to `true` for non-Docker runs). To stop, press `Ctrl+C`.
...
You should see `{"status":"ok","proxy":true}`.

> 📖 Detailed platform notes, troubleshooting, and configuration reference: see **[docs/direct-launch.md](./docs/direct-launch.md)**.
...
| 🪟 **Windows CMD** | `start.bat` / `npm start` | 本地 Windows，无 Docker |
| 🐧 **Linux** | `start.sh` / `npm start` | 本地 Linux，无 Docker |
| 🍎 **macOS** | `start.sh` / `npm start` | 本地 macOS，无 Docker |
| 💻 **独立 Node (manual backend)** | `MANAGE_BACKEND=false` 手动启动后端 | 开发、自定义集成 |
